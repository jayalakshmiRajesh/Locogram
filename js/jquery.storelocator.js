	...
	$.extend(Plugin.prototype, {
		...
		init: function () {
			...
			if (this.settings.modal === true) {
				...
				$('.' + this.settings.modalWindow).prepend('<div class="' + this.settings.closeIcon + '">Close<i>X</i></div>');
				$('.' + this.settings.modalContent).prepend('<h2>'+this.settings.merchant_name+'</h2>');
		...
		_formEventHandler: function () {
			...
			if (this.settings.noForm === true) {
				//stephen-- important: the formContainer name is used to show map on click of "View Map"
				$(document).on('click.'+pluginName, '.' + this.settings.formContainer + ' a', function (e) {
					_this.processForm(e);
				});
...
//				console.log(this.settings.data);
				// AJAX request -- stephen
				d.resolve(this.settings.data);
...
//				}).fail(d.reject);
		...
		processData: function (mappingObject, originPoint, data, page) {
			...
			for (var y = 0; y <= storeNumToShow - 1; y++) {
				...
				//stephen
				var point = new google.maps.LatLng(locationset[y].latitude, locationset[y].longitude);
