	...
	$.loader_ext = {
		...
		create : function() {
			...
			ops.imgUrl = ops.imgUrl
					.replace('[size]', ops.size + 'x' + ops.size);
		...
		// 
		set_css : function() {
			var scope = this.scope, ops = this.options, loading = this.loading, height = scope
					.outerHeight(), width = scope.outerWidth(), top = scope
					.offset().top, left = scope.offset().left;
			...
			loader.css({
				...
				'margin-left' : (width - ops.size) / 2 + ops.position[1]
						- loader.find('span').outerWidth() / 2
			});
		},
...
//			}
	...
	// 
	$.loader = {
		tmpl : '<div class="loading_wrp" id="${ID}" ><div class="loading ${Class}"><img src="${Src}" /><span>${Title}</span></div></div>',
